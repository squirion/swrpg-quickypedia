...
/// detail layout but with only 3 stat capsules (Price, Encumbrance,
/// Rarity) since gear has nothing else.
class GearViewScreen extends ConsumerWidget {
  final Gear gear;
  const GearViewScreen({super.key, required this.gear});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final all = ref.watch(gearProvider).asData?.value ?? const <Gear>[];
...
class _Hero extends ConsumerWidget {
  final Gear gear;
  const _Hero({required this.gear});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final hasImage =
        gear.imageUrl != null && gear.imageUrl!.isNotEmpty;
    final authHeaders =
        hasImage ? githubAuthHeadersFor(gear.imageUrl!, ref) : null;
          ...
          child: Stack(
            ...
            children: [
              ...
              Positioned(
                ...
                right: 18,
                bottom: 18,
                child: _HeroOverlay(gear: gear),
              ),
              Positioned(
                top: 10,
                right: 10,
                child: _EditImageButton(gear: gear),
              ),
...
class _EditImageButton extends ConsumerWidget {
  final Gear gear;
  const _EditImageButton({required this.gear});
...
class _HeroOverlay extends StatelessWidget {
  final Gear gear;
  const _HeroOverlay({required this.gear});
...
class _StatBlock extends StatelessWidget {
  final Gear gear;
  const _StatBlock({required this.gear});

  @override
  Widget build(BuildContext context) {
    final capsules = <(String, String?)>[
      ('PRICE', gear.price),
      ('ENCUM', gear.encumbrance),
      ('RARITY', gear.rarity),
    ];
