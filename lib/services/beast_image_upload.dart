...
class BeastImageUploader {
  static const int _maxEdgePx = 1024;
  static const String _imagesDir = 'images/beasts';
  static const String _beastsDbPath = 'databases/beasts.json';

  final GithubDataRepo _repo;
  final http.Client _client;

  BeastImageUploader(this._repo, {http.Client? client})
  ...
  Future<String> upload({
    required Beast beast,
    required Uint8List bytes,
  }) async {
    final png = _resizeToPng(bytes);
    final slug = slugFor(beast.name);
    ...
    final newSha = await _repo.putFile(
      path: path,
      bytes: png,
      commitMessage: 'Add image for ${beast.name}',
    ...
    final store = const SystemDataStore('beasts');
    final cached = await store.read<Beast>(Beast.fromJson);
    final updated = cached
        .map((b) => b.name == beast.name ? _withImageUrl(b, newUrl) : b)
        .toList(growable: false);
    await store.write<Beast>(updated, (b) => b.toJson());

    final dbBytes = await store.readBytes();
    if (dbBytes != null) {
      final currentDb = await _repo.getContents(_beastsDbPath);
      await _repo.putFile(
        path: _beastsDbPath,
        bytes: dbBytes,
        commitMessage: 'Set image for ${beast.name}',
  ...
  Future<String> uploadFromUrl({
    ...
    return upload(beast: beast, bytes: bytes);
  }

  void close() => _client.close();
}

Beast _withImageUrl(Beast b, String imageUrl) {
  final json = b.toJson();
