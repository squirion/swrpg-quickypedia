...
class GearSortMenu extends ConsumerWidget {
  const GearSortMenu({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final sort = ref.watch(gearSortProvider);
    return PopupMenuButton<GearSortAttr>(
      icon: const Icon(Icons.sort),
      tooltip: 'Sort',
      onSelected: (attr) =>
          ref.read(gearSortProvider.notifier).select(attr),
      itemBuilder: (_) => [
        for (final attr in GearSortAttr.values)
          PopupMenuItem<GearSortAttr>(
            value: attr,
            child: Row(
              children: [
                SizedBox(
