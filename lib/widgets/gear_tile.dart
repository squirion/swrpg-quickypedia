...
class GearTile extends ConsumerWidget {
  final Gear gear;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final rarity = _parseRarity(gear.rarity);
    ...
    final secondaryValue = secondaryAttr == null
        ? null
        : _displayValue(gear, secondaryAttr);

    final authHeaders = gear.imageUrl == null
        ? null
        : githubAuthHeadersFor(gear.imageUrl!, ref);
              ...
              children: [
                ...
                if (gear.price != null)
                  Positioned(
                    bottom: 6,
                    right: 8,
                    child: _PriceBadge(price: gear.price!),
                  ),
...
class _Background extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final url = imageUrl;
    if (url == null || url.isEmpty) return const GearPlaceholder();
...
String? _displayValue(Gear g, GearSortAttr attr) => switch (attr) {
